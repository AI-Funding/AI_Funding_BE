...
public class AiService {
    ...
    public static final LocalDateTime todayStartTime = LocalDateTime.of(LocalDate.now(), LocalTime.of(0, 0, 0));
    public static final LocalDateTime todayEndTime = LocalDateTime.of(LocalDate.now(), LocalTime.of(23, 59, 59));
    public static final LocalDateTime yesterdayStartTime = LocalDateTime.of(LocalDate.now().minusDays(1), LocalTime.of(0, 0, 0));
    public static final LocalDateTime yesterdayEndTime = LocalDateTime.of(LocalDate.now().minusDays(1), LocalTime.of(23, 59, 59));
    ...
    public List<StockInfoResponseDto> getStockDtoList(AccountEntity account) {
        ...
        //List<StockPriceByDayEntity> stockPriceByDay = stockPriceByDayRepository.findByStockEntityAndCreateAtBetween()

        //계좌Entity에 있는 모든 주식들을 하나씩 가져온다
        for (AccountStockDetailEntity stock : account.getAccountStocks()) {
            List<StockPriceByDayEntity> stockPriceByDayEntities = stockPriceByDayRepository.findByStockAndCreateAtBetween(stock.getStock(), yesterdayStartTime, yesterdayEndTime);
            ...
            Integer stockPriceChange = todayStockPrice * stock.getStockAmount() - yesterdayStockPrice * stock.getStockAmount();//어제주식가격 대비 상승가격
    ...
    public List<StockDetailResponseDto> getStockDetailDtoList(AccountStockDetailEntity stocks) {
        ...
        for (AccountStockDetailEntity stockDetail : stocks.getStock().getAccountStockDetailEntities()) {
            List<StockPriceByDayEntity> stockPriceByDayEntities = stockDetail.getStock().getStockPriceByDayEntities();
            StockPriceByDayEntity stockPriceByDay = stockPriceByDayEntities.get(stockPriceByDayEntities.size()-1);
